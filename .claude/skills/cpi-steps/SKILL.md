---
name: cpi-steps
description: Generate or reference flow step XML blocks for SAP CPI iFlows — Content Modifier, Groovy Script, XSLT/Message Mapping, Filter, Splitter, Aggregator, Request Reply, Exclusive Gateway with routing, Data Store operations, and full cmdVariantUri reference table. Use when adding or debugging flow steps inside an integration process.
...
def Message processData(Message message) {
    ...
    def body = message.getBody(String)  // use for plain text / JSON body access
...
**XML body — always use streaming (avoid CPI High warning):**
```groovy
// CORRECT — stream body into XmlSlurper (no CPI warning)
def xml = new XmlSlurper().parse(message.getBody(java.io.Reader))

// WRONG — CPI raises High warning: "parse the message body using XMLSlurper without streaming"
// def body = message.getBody(String)
// def xml = new XmlSlurper().parseText(body)
```

> CPI requires `message.getBody(java.io.Reader)` (or `java.io.Reader.class`) when parsing XML with XmlSlurper.
> Using `getBody(String)` + `parseText()` triggers a High severity design-time warning.
